...
class CtpnLoss(nn.Module):
    ...
    def forward(self, score, loc, score_target, loc_target):
        '''
        :param score: prediction score, shape=(1, 10*2, H, W)
        :param loc: prediction loc, shape=(1, 10*2, H, W)
        :param side_refinement:
        :param score_target: gt anchor label from rpn target. shape=(1, 10, H, W)
        :param loc_target: gt bbox regression target from rpn target. shape=(1, 40, H, W)
        :param side_refinement_target
        :return total loss
        '''
        # part1, classification loss
        score = score.reshape((score.shape[0], 2, -1, score.shape[-1])) # shape=(1, 2, 10H, W)
        ...
        score_target = score_target.permute((0, 3, 1, 2)) # (1, 10H, W, 1)
        ...
        loc = loc.reshape((loc.shape[0], 2, -1, loc.shape[-1]))
